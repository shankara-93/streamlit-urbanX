...
FASTAPI_BACKEND_URL = os.getenv("FASTAPI_BACKEND_URL", "https://urbanx-960048471565.europe-west4.run.app/")
        ...
        if show_parked_cars and intersecting_data is not None and not intersecting_data.empty and 'geometry' in intersecting_data.columns:
            ...
            if not valid_points_gdf.empty:
                ...
                def get_point_coords(geom):
                    ...
                    if geom.geom_type == 'MultiPoint' and not geom.is_empty: return (geom.geoms[0].y, geom.geoms[0].x) # Use first point of MultiPoint
                    return (None, None)
        ...
        if (show_all_cars or show_heatmap) and all_centroids_gdf is not None and not all_centroids_gdf.empty and 'geometry' in all_centroids_gdf.columns:
            ...
            if not valid_all_centroids_gdf.empty:
                ...
                if show_all_cars:
                    ...
                    for coords_set in all_coords_list: # Renamed to avoid conflict
                         all_car_locations.extend(coords_set)
                         all_car_popup_texts.extend(["Detected Car"] * len(coords_set))
...
def display_metrics(analysis_metadata: Dict) -> None:
    ...
    with st.container(height=325, border=True):
        ...
        with col2.container(border=False):
            ...
            except Exception as e: # Catch any other exception from style_metric_cards
                 logger.warning(f"Could not style metric cards: {e}")
            ...
            if gdf_occupied is not None and not gdf_occupied.empty:
                ...
                if available_columns:
                    ...
                    for col_name in available_columns:
                        ...
                        elif table_df[col_name].dtype in ['int64', 'float64', 'int32', 'float32']: # More robust type checking
                             column_config_dict[col_name] = st.column_config.NumberColumn(capitalized_label)
                        else: column_config_dict[col_name] = st.column_config.Column(label=capitalized_label) # Default for text/other
...
def display_sidebar_options(backend_configured: bool) -> Tuple[str, bool, Dict[str, Any], st.empty]:
    ...
    google_drive_url = st.sidebar.text_input(
        ...
        disabled=not backend_configured  # Disable if backend is not configured
    )
    ...
    process_button_clicked = st.sidebar.button(
        ...
        disabled=not backend_configured or not google_drive_url # Disable if no URL or backend not configured
    )

    if not backend_configured:
        status_placeholder.error("⚠️ Backend URL not configured. Processing disabled.")
    elif not google_drive_url and not st.session_state.get("show_api_results", False): # Initial state check
        status_placeholder.info("ℹ️ Provide a Google Drive URL, then click 'Process'.")
    # Other status updates will be handled in main()
...
# --- Main Application (Updated with Session State) ---
def main() -> None:
    set_page_config()
    load_logo()

    # Initialize session state
    if "api_processed_data" not in st.session_state:
        st.session_state.api_processed_data = None
    if "show_api_results" not in st.session_state:
        st.session_state.show_api_results = False
    if "last_processed_url_api" not in st.session_state:
        st.session_state.last_processed_url_api = ""
        
    backend_configured = FASTAPI_BACKEND_URL != "https://urbanx-960048471565.europe-west4.run.app"


    if not backend_configured:
        st.error(
            "🚨 Critical Configuration Error: The FastAPI backend URL is not configured. "
            "Please set the `FASTAPI_BACKEND_URL` environment variable or update it directly in the script."
        )
        # The script will continue to render the sidebar at least

    google_drive_url, process_button_clicked, map_options, status_placeholder = display_sidebar_options(backend_configured)

    # --- Logic Triggered by Button Click ---
    if process_button_clicked:

        if google_drive_url != st.session_state.last_processed_url_api:
            st.session_state.show_api_results = False
            st.session_state.api_processed_data = None
            st.session_state.last_processed_url_api = google_drive_url

        if google_drive_url: # This check is redundant if button is disabled correctly, but safe
            data_source_name = f"Google Drive URL: ...{google_drive_url[-30:]}"
            status_placeholder.info(f"Processing: {data_source_name}")
            logger.info(f"Processing started for URL: {google_drive_url}")

            endpoint = f"{FASTAPI_BACKEND_URL}/detect-from-url"
            api_response_json = call_fastapi_from_url(endpoint, google_drive_url) # This function should show st.error on failure
            if isinstance(api_response_json, dict):
                 logger.info(f"--- DEBUG: API response (first 100 chars): {str(api_response_json)[:100]}")
            elif api_response_json is not None: # e.g. if it's a list or other non-dict type (should be dict)
                 logger.info(f"--- DEBUG: API response (unexpected type): {api_response_json}")


            if api_response_json: # If API call was successful and returned something
                ...
                if analysis_metadata is not None: # Crucial check
                    ...
                    st.session_state.api_processed_data = {
                        ...
                        "gdf_empty": parsed_data[2] if parsed_data and len(parsed_data) > 2 else None,
                        "image_bbox": parsed_data[3] if parsed_data and len(parsed_data) > 3 else None,
                        "gdf_image_centers": parsed_data[4] if parsed_data and len(parsed_data) > 4 else None,
                        "data_source_name": data_source_name
                    }
                    st.session_state.show_api_results = True
                    status_placeholder.success(f"✓ Processing complete for {data_source_name}")
                    logger.info(f"Application successfully parsed and stored results for {data_source_name}.")
                else:
                    st.write("--- DEBUG: Analysis metadata IS None after parsing. Not storing results in session. Display errors should have occurred in parse_api_response.")
                    st.session_state.api_processed_data = None # Ensure it's cleared
                    st.session_state.show_api_results = False # Ensure results are not shown
                    status_placeholder.error(f"❌ Failed to process/parse results for {data_source_name}")
                    # logger.error already done by parse_api_response if it returned None for metadata
            else:
                # API call itself failed or returned None. call_fastapi_from_url should have shown an st.error.
                st.write("--- DEBUG: API response IS None. Not processing further.")
                st.session_state.api_processed_data = None
                st.session_state.show_api_results = False
                status_placeholder.error(f"❌ Failed to get results for {data_source_name}") # This might be redundant if call_fastapi logs
        else:
            # This case should ideally not be reached if the button is correctly disabled when no URL.
            st.write("--- DEBUG: Process button clicked, but no Google Drive URL was found (this is unexpected if button disabled).")
            if backend_configured: # Only show warning if backend is ok but no URL
                 status_placeholder.warning("⚠️ No Google Drive URL provided.")


    # --- Display results if available in session state ---
    if st.session_state.show_api_results and st.session_state.api_processed_data:
        #st.write("--- DEBUG: Conditions met. Attempting to display results header.")
        data = st.session_state.api_processed_data
        st.header("Analysis Results") # This should appear if we get here
        
        try:
            display_metrics(data["analysis_metadata"])
        except Exception as e_metrics:
            st.error(f"--- DEBUG: Error in display_metrics: {e_metrics}")
            logger.error(f"Exception in display_metrics: {e_metrics}", exc_info=True)

        try:
            display_map_and_insights(
                data.get("gdf_occupied"), # Use .get for safety
                data.get("gdf_empty"),
                data.get("image_bbox"),
                data.get("gdf_image_centers"),
                map_options
            )
        except Exception as e_map:
            st.error(f"--- DEBUG: Error in display_map_and_insights: {e_map}")
            logger.error(f"Exception in display_map_and_insights: {e_map}", exc_info=True)
    else:
        logger.info("--- DEBUG: Conditions NOT met for displaying results OR no data processed yet.")
        # Show an initial message if nothing has been processed and no error state from backend config
        if not st.session_state.show_api_results and not process_button_clicked and backend_configured :
            if not google_drive_url: # Only if no URL has been entered yet
                 st.info("Enter a Google Drive URL and click 'Process Image' to see results.")
            # If URL is entered but not processed, status_placeholder handles it or errors from processing did.
